...
class MyBiLSTM(nn.Module):
    ...
    def forward(self, x, idxs):
        ...
        temp = torch.randn(0, 128).to(device)
        for i in range(batch_size):
            select = torch.index_select(h[i], dim=0, index=torch.tensor(idxs[i]).to(device))
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        # 训练
        model.train()
        for i, (x, y, idxs, ids) in enumerate(train_loader):
            model.zero_grad()

            h, y_hat = model(x.to(device), idxs)
            y = y.to(device)
        ...
        model.eval()
        with torch.no_grad():
            for i, (x, y, idxs, ids) in enumerate(val_loader):
                h, y_hat = model(x.to(device), idxs)
                y = y.to(device)
                loss = loss_function(y_hat, y)
    ...
    model.eval()
    with torch.no_grad():
        for i, (x, y, idxs, ids) in enumerate(test_loader):
            h, y_hat = model(x.to(device), idxs)
            y = y.to(device)
            loss = loss_function(y_hat, y)
